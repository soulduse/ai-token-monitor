...
- **隐私优先** — 默认 100% 离线;Cursor 公开资料、排行榜、聊天和 Webhook 均为严格可选。
...
以上全部在你的电脑上完成。除非你主动开启 Cursor 公开资料、排行榜、聊天或 Webhook,应用**不会发出任何网络请求**。Cursor 功能只读取你配置的公开资料 URL；共享功能只发送汇总数据,绝不会共享代码或对话内容。
...
**网络请求**:仅在开启 Cursor 公开资料(读取已配置的公开页面)、排行榜/聊天(向 Supabase 发送汇总数据)、触发 Webhook 或配置 AI 翻译提供商时发起。不使用这些功能时,应用完全离线运行。
