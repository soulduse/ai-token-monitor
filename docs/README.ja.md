...
- **プライバシー第一の設計** — デフォルトで 100% オフライン。Cursor 公開プロフィール・リーダーボード・チャット・Webhook は完全にオプトイン。
...
これらはすべてユーザーのマシン上で完結します。Cursor 公開プロフィール・リーダーボード・チャット・Webhook を有効にしない限り、アプリは**一切ネットワークリクエストを送信しません**。Cursor は設定した公開プロフィール URL のみを読み、共有機能が送信するのは集計値だけで、コードや会話内容は共有されません。
...
**ネットワークリクエスト**：Cursor 公開プロフィール有効時（設定した公開ページの読み取り）、リーダーボード/チャットのオプトイン時（Supabase への集計データ送信）、Webhook 発火時、または AI 翻訳プロバイダー設定時にのみ送信されます。これらを使わなければ、アプリは完全にオフラインで動作します。
