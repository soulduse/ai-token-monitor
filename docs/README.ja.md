...
**AI Token Monitor** は「自分の AI コーディングツールは実際いくらかかっているのか?」という質問に一日中答えてくれる、macOS / Windows 向けの軽量なシステムトレイアプリです。**Claude Code**、**Codex**、**OpenCode**、**GJC**、**Grok** がすでに書き出しているローカルのセッションログを読み取り、モデル別単価(キャッシュ読み取り含む)ですべてのトークンのコストを計算し、今日の支出を時計のすぐ隣に表示します。チャート、プラン上限アラート、オプトイン式のリーダーボード・チャット・Webhook 通知もワンクリックで開けます。

- **セットアップ不要** — API キーもプロキシも不要。Claude Code や Codex を一度でも実行していれば、すぐに動きます。
- **支出がひと目でわかる** — メニューバー / システムトレイにリアルタイムのコストを表示、クリックでフルダッシュボード。
- **上限の一歩先へ** — 5 時間セッションと週間プラン上限バーをライブ表示し、上限に達する前に Discord / Slack / Telegram へ通知。
- **プライバシー第一の設計** — デフォルトで 100% オフライン。リーダーボード・チャット・Webhook は完全にオプトイン。
...
![AI Token Monitor の仕組み — ローカルのセッションログを読み取り、ローカルで解析・課金計算し、トレイとダッシュボードに表示](images/how-it-works.png)

1. **ローカルのセッションログを読み取り** — AI CLI がすでに書き出している JSONL ファイルを監視します(正確なパスは[データソース](#データソース)を参照)
2. **ローカルで解析 & 課金計算** — Rust エンジンがエントリを重複排除し、モデル別単価(キャッシュ読み取り含む)を適用、ファイルが変わった瞬間に再集計します
3. **あらゆる場所に表示** — トレイのコスト表示、ダッシュボードのチャート、プラン上限アラートバー、そして任意の Webhook 通知

これらはすべてユーザーのマシン上で完結します。リーダーボード・チャット・Webhook にオプトインしない限り、アプリは**一切ネットワークリクエストを送信せず**、オプトインした場合も共有されるのは集計値のみで、コードや会話の内容が共有されることはありません。

## 主な機能

![機能ハイライト — 追跡と可視化、競争と共有、予算管理、カスタマイズ](images/features.png)
