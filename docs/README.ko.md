...
**AI Token Monitor**는 "내 AI 코딩 도구가 실제로 얼마나 쓰고 있지?"라는 질문에 하루 종일 답해주는 가벼운 macOS / Windows 시스템 트레이 앱입니다. **Claude Code**, **Codex**, **OpenCode**, **GJC**, **Grok**, **Kiro**가 이미 남기고 있는 로컬 세션 로그를 읽어 모델별 단가(캐시 읽기 포함)로 모든 토큰의 비용을 계산하고, 오늘의 지출을 시계 바로 옆에 보여줍니다. 차트, 플랜 한도 알림, 옵트인 리더보드·채팅·웹훅 알림까지 클릭 한 번 거리에 있습니다.

- **설정 제로** — API 키도 프록시도 필요 없습니다. Claude Code나 Codex를 한 번이라도 실행했다면 바로 동작합니다.
- **지출을 한눈에** — 메뉴바/시스템 트레이에 실시간 비용 표시, 클릭하면 전체 대시보드가 열립니다.
- **한도보다 한발 앞서** — 5시간 세션·주간 플랜 한도 바를 실시간 표시하고, 한도에 닿기 전에 Discord / Slack / Telegram으로 알립니다.
- **프라이버시 우선 설계** — 기본은 100% 오프라인. Cursor 공개 프로필·리더보드·채팅·웹훅은 철저히 옵트인입니다.
...
![AI Token Monitor 작동 방식 — 로컬 세션 로그 읽기, 로컬에서 파싱·과금 계산, 트레이와 대시보드에 표시](images/how-it-works.png)

1. **로컬 세션 로그 읽기** — AI CLI가 이미 남기고 있는 JSONL 파일을 감시합니다 (정확한 경로는 [데이터 소스](#데이터-소스) 참고)
2. **로컬에서 파싱 & 과금 계산** — Rust 엔진이 항목을 중복 제거하고 모델별 단가(캐시 읽기 포함)를 적용하며, 파일이 변경되는 즉시 재집계합니다
3. **어디서나 표시** — 트레이 비용 표시, 대시보드 차트, 플랜 한도 알림 바, 그리고 선택적 웹훅 알림

이 모든 과정은 사용자의 컴퓨터 안에서 이루어집니다. Cursor 공개 프로필·리더보드·채팅·웹훅을 활성화하지 않는 한 앱은 **네트워크 요청을 전혀 보내지 않습니다**. Cursor 기능은 설정한 공개 프로필 URL만 읽고, 공유 기능은 집계된 수치만 전송하며 코드나 대화 내용은 절대 공유하지 않습니다.

## 주요 기능

![기능 하이라이트 — 추적 & 시각화, 경쟁 & 공유, 예산 관리, 커스터마이징](images/features.png)
...
**네트워크 요청**: Cursor 공개 프로필 활성화 시 설정한 공개 페이지를 읽고, 리더보드/채팅 opt-in 시 Supabase로 집계 데이터를 전송하며, 웹훅 발화 또는 AI 번역 프로바이더 설정 시 외부 요청을 보냅니다. 이 기능을 쓰지 않으면 앱은 완전히 오프라인으로 동작합니다.
