-- Extend the provider whitelist on daily_snapshots to accept 'gjc' (Gajae Code).
--
-- The client-side GJC provider was added alongside this migration, but the
-- Supabase layer still rejects 'gjc' at two points:
--   1. the daily_snapshots_provider_check CHECK constraint
--   2. the sync_device_snapshots() RPC's provider validation
-- Without this migration, leaderboard uploads for GJC silently fail at the RPC
-- boundary with 'Invalid provider'.
--
-- The function body below is copied verbatim from
-- 20260417040000_add_kimi_glm_providers.sql (the current bulk-UPSERT v2
-- implementation) with only the `p_provider not in (...)` list widened to add
-- 'gjc'. Keeping the rest identical avoids accidental behavior drift.
...
check (provider in ('claude', 'codex', 'opencode', 'kimi', 'glm', 'gjc'));
...
begin
  ...
  if p_provider not in ('claude', 'codex', 'opencode', 'kimi', 'glm', 'gjc') then
