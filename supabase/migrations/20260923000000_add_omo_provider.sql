-- Extend the provider whitelist on daily_snapshots to accept 'omo' (OmO Native,
-- oh-my-openagent on senpi).
--
-- OmO is token-based like claude/codex, so no unit change is needed.
--
...
-- useSnapshotUploader swallows the error (`return !error`), so nothing would
-- surface in the UI without this migration.
--
-- The body below is copied verbatim from
-- 20260731000000_add_kiro_provider.sql with only the provider lists widened.
...
check (provider in ('claude', 'codex', 'opencode', 'kimi', 'glm', 'gjc', 'grok', 'kiro', 'omo'));
...
begin
  ...
  if p_provider not in ('claude', 'codex', 'opencode', 'kimi', 'glm', 'gjc', 'grok', 'kiro', 'omo') then
