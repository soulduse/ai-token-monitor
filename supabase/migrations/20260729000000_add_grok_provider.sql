-- Extend the provider whitelist on daily_snapshots to accept 'grok' (xAI Grok).
--
-- The client-side Grok provider was added alongside this migration, but the
-- Supabase layer still rejects 'grok' at two points:
--   1. the daily_snapshots_provider_check CHECK constraint
--   2. the sync_device_snapshots() RPC's provider validation
-- Without this migration, leaderboard uploads for Grok silently fail at the RPC
-- boundary with 'Invalid provider' -- and useSnapshotUploader swallows the error
-- (`return !error`), so nothing surfaces in the UI.
--
-- The function body below is copied verbatim from
-- 20260627000000_add_gjc_provider.sql (the current bulk-UPSERT v2
-- implementation) with only the `p_provider not in (...)` list widened to add
-- 'grok'. Keeping the rest identical avoids accidental behavior drift.
...
check (provider in ('claude', 'codex', 'opencode', 'kimi', 'glm', 'gjc', 'grok'));
...
begin
  ...
  if p_provider not in ('claude', 'codex', 'opencode', 'kimi', 'glm', 'gjc', 'grok') then
